...
def filter_peaks(spec, freqs, thresh_amp_low, thresh_amp_high, thresh_freq_low, thresh_freq_high, prominence=None, width=None):
    peaks, _ = find_peaks(spec, height=(thresh_amp_low, thresh_amp_high), prominence=prominence, width=width)
...
def plot_spectrum(freqs, spec, peaks, thresh_amp_low, thresh_amp_high, thresh_freq_low, thresh_freq_high):
    ...
    fig.add_shape(
        ...
        y0=0,
        x1=thresh_freq_low,
        y1=thresh_amp_high + 0.05 * thresh_amp_high,
        line=dict(
            color="Red",
            width=2,
            dash="dash",
...
def analyze_signal(signal, sr, filename, window_type='boxcar', thresh_amp_low=0.2, thresh_amp_high=0.4, thresh_freq_low=0, thresh_freq_high=None, prominence=None, width=None, show_peaks=False, show_plot=True):
    if window_type:
        signal = apply_window(signal, window_type)
    
    freqs, spec = compute_fft(signal, sr)
    peaks = filter_peaks(spec, freqs, thresh_amp_low, thresh_amp_high, thresh_freq_low, thresh_freq_high, prominence, width)
